...
import { Home, Admin, Signin, Contents, UpdateCpntents, Users, Distribtors } from "./pages";
  ...
  return (
    <Router>
      <Routes> 
        ...
        <Route exact path={ROUTERS.USERS} element={<ProtectedRoute user={user}>{<Users />}</ProtectedRoute>}/>
        <Route exact path={ROUTERS.CONTENTS} element={<ProtectedRoute user={user}>{<Contents />}</ProtectedRoute>}/>
        <Route exact path={ROUTERS.DISTRIBUTORS} element={<ProtectedRoute user={user}>{<Distribtors />}</ProtectedRoute>}/>
